...
public class MainActivity extends Activity {
    ...
    Button btnConnect,btnSend,btnQuit,test;
    EditText etReceived,etSend;
    MySurfaceView move;
    MySurfaceView_copy turn;
    ...
    private static String address = "B0:6E:BF:51:19:C2"; // <==要连接的目标蓝牙设备MAC地址
    private ReceiveThread rThread=null;  //数据接收线程

    //接收到的字符串
    String ReceiveData="";
    MyHandler handler;
    static int movesymbol=5;
    static int turnsymbol=4;
    ...
    public void Init()
    {
        statusLabel=(TextView)this.findViewById(R.id.tip);
        btnConnect=(Button)this.findViewById(R.id.connect);
        btnQuit=(Button)this.findViewById(R.id.disconnect);
        move=(MySurfaceView)this.findViewById((R.id.movecontrol));//前进按钮
        turn=(MySurfaceView_copy)this.findViewById((R.id.turncontrol));//转向按钮
    }
    ...
    public void InitBluetooth()
    {
        ...
        if(mBluetoothAdapter == null)
        {
            ...
            finish();//将活动推向后台，并没有立即释放内存，活动的资源并没有被清理
            return;
        }
    }
        ...
        protected String doInBackground(String... arg0) {
            ...
            if(arg0[0].length()>0)//不是空白串
            {
                ...
                try {
                    ...
                    outStream.write(movesymbol);
                    outStream.write(turnsymbol);
...
//                        System.out.println("buff receive :"+buff.length);
                    if(move.SmallRockerCircleY<300)
                    {

                        if(movesymbol!=2){
                            System.out.println("前进");
                            movesymbol=2;
                            new SendInfoTask().execute("5555");
                        }
                    }
                    if(move.SmallRockerCircleY>300)
                    {
                    ...
                    if(turn.SmallRockerCircleX<300)
                    {
                        if(turnsymbol!=1){
